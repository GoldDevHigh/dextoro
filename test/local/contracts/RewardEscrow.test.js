...
const TokenContract = artifacts.require("DexToro");
const RewardsEscrow = artifacts.require("RewardEscrow");

const NAME = "DexToro";
const SYMBOL = "DTORO";
const INITIAL_SUPPLY = hre.ethers.utils.parseUnits("25000000");
...
contract(
    ...
    ([owner, staker1, staker2, treasuryDAO]) => {
        ...
        before(async () => {
            dextoroSmock = await smock.fake("DexToro");
            ...
            rewardsEscrow = await RewardsEscrow.new(owner, dextoroSmock.address);

            stakingRewards = await StakingRewards.new(
                dextoroSmock.address,
        ...
        describe("Deploys correctly", async () => {
            it("Should have a DTORO token", async () => {
                const dextoroAddress = await rewardsEscrow.getDexToroAddress();
                assert.equal(
                    dextoroAddress,
                    dextoroSmock.address,
        ...
        describe("Writing vesting schedules()", async () => {
            it("should not create a vesting entry with a zero amount", async () => {
                // Transfer of DTORO to the escrow must occur before creating an entry
            ...
            it("should not create a vesting entry if there is not enough DTORO in the contracts balance", async () => {
                // Transfer of DTORO to the escrow must occur before creating an entry
                await stakingToken.transfer(
                    rewardsEscrow.address,
                    toUnit("1"),
        ...
        describe("Creating vesting Schedule", async () => {
            describe("When appending vesting entry via feePool", async () => {
                ...
                it("should revert appending a vesting entry from staker1", async () => {
                    // Transfer of DTORO to the escrow must occur before creating an entry
                    dextoroSmock.balanceOf.returns(wei(10).toBN());
                ...
                it("should revert appending a vesting entry with a zero amount", async () => {
                    // Transfer of DTORO to the escrow must occur before creating an entry
                    dextoroSmock.balanceOf.returns(wei(1).toBN());
                ...
                it("should revert appending a vesting entry if there is not enough DTORO in the contracts balance", async () => {
                    // Transfer of DTORO to the escrow must occur before creating an entry
                    dextoroSmock.balanceOf.returns(wei(1).toBN());
                ...
                describe("When successfully appending new escrow entry for account 1 with 10 DTORO", () => {
                    ...
                    it("Account 1 should have balance of 10 DTORO", async () => {
                        assert.bnEqual(
                            await rewardsEscrow.balanceOf(staker1),
                            escrowAmount
                        );
                    });
                    it("totalEscrowedBalance of the contract should be 10 DTORO", async () => {
                        assert.bnEqual(
                    ...
                    describe("When 6 months has passed", () => {
                        ...
                        it("then the vesting entry has 5.5 dextoro claimable", async () => {
                            const claimable =
        ...
        describe("Creating a new escrow entry by approval", async () => {
            ...
            beforeEach(async () => {
                // approve rewardEscrow to spend DTORO
                dextoroSmock.allowance.returns(wei(10).toBN());

                // stub transferFrom
                dextoroSmock.transferFrom.returns(true);
            ...
            it("should revert when msg.sender has no approval to spend", async () => {
                dextoroSmock.transferFrom.returns(false);
                await assert.revert(
                    rewardsEscrow.createEscrowEntry(
                        staker1,
                        toUnit("10"),
            ...
            describe("when successfully creating a new escrow entry for staker1", () => {
                ...
                it("Account1 should have balance of 10 DTORO", async () => {
                    assert.bnEqual(
                        await rewardsEscrow.balanceOf(staker1),
                        escrowAmount
                    );
        ...
        describe("Read Vesting Schedule", () => {
            ...
            beforeEach(async () => {
                // Transfer of DTORO to the escrow must occur before creating a vestinng entry
                dextoroSmock.balanceOf.returns(wei(1000).toBN());
        ...
        describe("Vesting", () => {
            ...
            beforeEach(async () => {
                mockedDexToro = await TokenContract.new(
                    NAME,
                    SYMBOL,
                    INITIAL_SUPPLY,
                    owner,
                ...
                await mockedDexToro.transfer(owner, toUnit("1000"), {
                    from: treasuryDAO,
                });

                // Transfer of DTORO to the escrow must occur before creating a vesting entry
                await mockedDexToro.transfer(
                    rewardsEscrow.address,
                    toUnit("1000"),
                    {
                        from: owner,
            ...
            describe("Vesting of vesting entry after 6 months (before escrow ends)", () => {
                ...
                let escrowAmount, timeElapsed, entryID, claimableDTORO;
                beforeEach(async () => {
                ...
                it("should vest 0 amount if entryID does not exist for user", async () => {
                    ...
                    // Check user has no vested DTORO
                    assert.bnEqual(
                        await mockedDexToro.balanceOf(staker1),
                        toUnit("0")
                    );

                    // Check rewardEscrow does not have any DTORO
                    assert.bnEqual(
                        await mockedDexToro.balanceOf(rewardsEscrow.address),
                        escrowAmount
                    );
                ...
                it("should vest and transfer 0 DTORO from contract to the user", async () => {
                    claimableDTORO =
                        await rewardsEscrow.getVestingEntryClaimable(
                            staker1,
                            entryID
                    ...
                    const treasuryPreBalance = await mockedDexToro.balanceOf(
                        treasuryDAO
                    );
                    ...
                    const treasuryPostBalance = await mockedDexToro.balanceOf(
                        treasuryDAO
                    );

                    // Check user has more than 55% vested DTORO
                    assert.bnClose(
                        await mockedDexToro.balanceOf(staker1),
                        wei(escrowAmount).mul(0.55).toString(0),
                        wei(1).toString()
                    ...
                    // Check treasury has 40% vested DTORO
                    assert.bnClose(
                    ...
                    // Check rewardEscrow contract has same amount of DTORO
                    assert.bnEqual(
                        await mockedDexToro.balanceOf(rewardsEscrow.address),
                        0
                    );
            ...
            describe("When vesting after escrow ended", () => {
                ...
                it("should vest and transfer all the $DTORO to the user", async () => {
                    await rewardsEscrow.vest([entryID], {
                        from: staker1,
                    });

                    // Check user has all their vested DTORO
                    assert.bnEqual(
                        await mockedDexToro.balanceOf(staker1),
                        escrowAmount
                    );
            ...
            describe("Vesting multiple vesting entries", () => {
                ...
                describe("When another user (account 1) vests all their entries", () => {
                    it("should vest all entries and transfer $DTORO to the user", async () => {
                        ...
                        // Check staker1 has no DTORO in their balance
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(staker1),
                            toUnit("0")
                        );

                        // Check staker2 has no DTORO in their balance
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(staker2),
                            toUnit("0")
                        );

                        // Check rewardEscrow has all the DTORO
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(rewardsEscrow.address),
                            toUnit("1000")
                        );
                ...
                it("should vest all entries and transfer $DTORO from contract to the user", async () => {
                ...
                it("should vest all entries and ignore duplicate attempts to vest same entries again", async () => {
                    ...
                    // Check user has same amount of DTORO
                    assert.bnEqual(
                        await mockedDexToro.balanceOf(staker1),
                        toUnit("1000")
                    );
            ...
            describe("Vesting multiple vesting entries with different duration / end time", () => {
                ...
                describe("vest only the first two entries", () => {
                    ...
                    it("should vest only first 2 entries and transfer $DTORO from contract to the user", async () => {
                    ...
                    it("should vest and update totalEscrowedAccountBalance", async () => {
                        ...
                        // This account should have any 500 DTORO escrowed
                        escrowedAccountBalance =
                            await rewardsEscrow.totalEscrowedAccountBalance(
                                staker1
                            );
                    ...
                    it("should vest entryID1 and entryID2 and ignore duplicate attempts to vest same entries again", async () => {
                        ...
                        // Check user have vested escrowAmount1 and escrowAmount2 DTORO
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(staker1),
                            escrowAmount1.add(escrowAmount2)
                        );

                        // Check rewardEscrow does has escrowAmount3 DTORO
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(rewardsEscrow.address),
                            escrowAmount3
                        );
                        ...
                        // Check rewardEscrow has same escrowAmount3 DTORO
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(rewardsEscrow.address),
                            escrowAmount3
                        );
                ...
                describe("when the first two entries are vestable and third is partially vestable", () => {
                    ...
                    it("should vest and update totalEscrowedAccountBalance", async () => {
                        ...
                        // This account should have any 0 DTORO escrowed
                        escrowedAccountBalance =
                            await rewardsEscrow.totalEscrowedAccountBalance(
                                staker1
                            );
                    ...
                    it("should vest entryID1 and entryID2 and ignore duplicate attempts to vest same entries again", async () => {
                        ...
                        const beforeBalance = await mockedDexToro.balanceOf(
                            staker1
                        );

                        // Check user have vested escrowAmount1 + escrowAmount2 + partial escrowAmount3 DTORO
                        assert.bnGreaterThan(
                            beforeBalance,
                            escrowAmount1.add(escrowAmount2)
                        );
                        ...
                        // Check rewardEscrow has same 0 DTORO
                        assert.bnEqual(
                            await mockedDexToro.balanceOf(rewardsEscrow.address),
                            0
                        );
        ...
        describe("Staking Escrow", () => {
            ...
            it("should revert because staker has no escrowed DTORO", async () => {
            ...
            it("should revert because staker does not have enough escrowed DTORO", async () => {
                const escrowAmount = wei(10).toBN();
                // stub transferFrom
                dextoroSmock.transferFrom.returns(true);
                // stub balanceOf
                dextoroSmock.balanceOf.returns(escrowAmount);
                await rewardsEscrow.createEscrowEntry(
                    staker1,
                    escrowAmount,
                    1 * YEAR
            ...
            it("should vest without unstaking escrow", async () => {
                ...
                dextoroSmock.transferFrom.returns(true);
                // stub balanceOf
                dextoroSmock.balanceOf.returns(escrowAmount.mul(2));
                await rewardsEscrow.createEscrowEntry(
                    staker1,
                    escrowAmount,
                    1 * YEAR
            ...
            it("should unstake escrow to partially vest, partially impose fee and send to treaury", async () => {
                ...
                // stub transferFrom
                dextoroSmock.transferFrom.returns(true);
                dextoroSmock.transfer.returns(true);
                // stub balanceOf
                dextoroSmock.balanceOf.returns(escrowAmount);
                await rewardsEscrow.createEscrowEntry(
                    staker1,
