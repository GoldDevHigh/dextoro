...
const deployRewardEscrowV2 = async (owner, dextoro, rewardsNotifier) => {
    ...
    const rewardEscrowV2 = await upgrades.deployProxy(
        ...
        {
            kind: "uups",
            constructorArgs: [dextoro, rewardsNotifier],
        }
    );
...
contract("RewardEscrowV2 DTORO", ([owner, staker1, staker2, treasuryDAO]) => {
    ...
    describe("Deploys correctly", async () => {
        it("Should have a DTORO token", async () => {
            const dextoroAddress = await rewardEscrowV2.getDexToroAddress();
            assert.equal(
                dextoroAddress,
                stakingToken.address,
