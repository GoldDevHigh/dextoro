...
/**
 * @notice deploy contracts and transfer 1/4 supply of dextoro to staking rewards
 */
...
/**
 * @notice using DexToro.sol contract as a mock token for testing.
 * Please notice: mockToken != dextoro
 */
const deployMockToken = async () => {
    const MockToken = await ethers.getContractFactory("DexToro");
...
/**
 * @noitce transfer dextoro to rewardEscrow contract and approve StakingRewards to spend amount
 * @param amount - amount to transfer and approve
 */
const fundAndApproveRewardEscrow = async (amount: number) => {
    // fund rewardEscrow
    await dextoro.connect(TREASURY_DAO).transfer(rewardEscrow.address, amount);
...
/**
 * @noitce transfer dextoro to account and approve StakingRewards to spend amount
...
const fundAndApproveAccount = async (
    ...
    await dextoro.connect(TREASURY_DAO).transfer(account.address, amount);
    await dextoro.connect(account).approve(stakingRewards.address, amount);
};

/**
 * @notice stakingRewards is funded with reward tokens (DTORO) when
 * setupStakingRewards() is called
 * @notice mockToken is based on dextoro.sol, but IS NOT dextoro
...
describe("StakingRewards", () => {
    describe("Constructor & Settings", () => {
        ...
        it("should set staking/rewards token on constructor", async () => {
            expect(await stakingRewards.token()).to.equal(dextoro.address);
        });
    ...
    describe("Function Permissions", () => {
        ...
        it("only owner address can call recoverERC20", async () => {
            let tx = stakingRewards
                .connect(addr1)
                .recoverERC20(dextoro.address, 0);
            await expect(tx).to.be.revertedWith(
                "Only the contract owner may perform this action"
            );

            // notice following tx passes owner check despite ultimately failing
            tx = stakingRewards.connect(owner).recoverERC20(dextoro.address, 0);
            await expect(tx).to.be.revertedWith(
    ...
    describe("External Rewards Recovery", () => {
        ...
        it("should revert if recovering staking token", async () => {
            let tx = stakingRewards
                .connect(owner)
                .recoverERC20(dextoro.address, TEST_VALUE);
            await expect(tx).to.be.revertedWith(
    ...
    describe("stake()", () => {
        ...
        it("staking increases token balance", async () => {
            const preBal = await dextoro.balanceOf(stakingRewards.address);

            // stake
            await stakingRewards.connect(addr1).stake(SECONDS_IN_WEEK);

            const postBal = await dextoro.balanceOf(stakingRewards.address);
    ...
    describe("stakeEscrow()", () => {
        ...
        it("stake, stake escrow, and then call exit()", async () => {
            ...
            expect(await dextoro.balanceOf(addr1.address)).to.equal(
                nonEscrowStakedBal
            );
        });
    });
    ...
    describe("getReward()", () => {
         ...
         */
        it("should increase dextoro balance in escrow", async () => {
            const preEscrowBalance = await dextoro.balanceOf(
                rewardEscrow.address
            );
            ...
            const postEscrowBalance = await dextoro.balanceOf(
                rewardEscrow.address
            );
    ...
    describe("unstake()", () => {
        ...
        it("should increases lp token balance and decreases staking balance", async () => {
            ...
            const initialTokenBal = await dextoro.balanceOf(addr1.address);
            ...
            const postTokenBal = await dextoro.balanceOf(addr1.address);
    ...
    describe("unstakEscrow()", () => {
        ...
        it("should not change token balance(s) for lp account nor contract", async () => {
            ...
            const preAddr1Bal = await dextoro.balanceOf(addr1.address);
            const preSRBalance = await dextoro.balanceOf(stakingRewards.address);
            ...
            const postAddr1Bal = await dextoro.balanceOf(addr1.address);
            const postSRBalance = await dextoro.balanceOf(
    ...
    describe("exit()", () => {
        ...
        it("should retrieve all earned and increase rewards bal", async () => {
            ...
            const initialRewardBal = await dextoro.balanceOf(addr1.address);
            const initialEarnedBal = await stakingRewards.earned(addr1.address);
            await stakingRewards.connect(addr1).exit();
            const postRewardBal = await dextoro.balanceOf(addr1.address);
    ...
    describe("Known Edge Case(s)", () => {
        ...
        it.skip("Inefficient Reward Distribution", async () => {
            ...
            expect(await dextoro.balanceOf(rewardEscrow.address)).to.equal(0);
            await stakingRewards.connect(addr1).getReward();
            expect(await dextoro.balanceOf(rewardEscrow.address)).to.equal(
