...
import { DexToro } from "../../../typechain/DexToro";
...
describe("SupplySchedule", () => {
    const NAME = "DexToro";
    const SYMBOL = "DTORO";
    const INITIAL_SUPPLY = wei(25000000);
    ...
    const initialWeeklySupply = wei(25000000).mul(2.4).div(52);
    let inflationStartDate: number;

    let accounts: SignerWithAddress[];
    let owner: SignerWithAddress, account2: SignerWithAddress;
    let supplySchedule: SupplySchedule, dextoro: DexToro;
    ...
    describe("linking dextoro", async () => {
        it("should have set dextoro", async () => {
            expect(await supplySchedule.dextoro()).to.equal(dextoro.address);
        });
        it("should revert when setting dextoro to ZERO_ADDRESS", async () => {
            await expect(supplySchedule.setDexToro(AddressZero)).to.be.reverted;
        });

        it("should emit an event when setting dextoro", async () => {
            await expect(
                supplySchedule.connect(owner).setDexToro(account2.address)
            )
                .to.emit(supplySchedule, "DexToroUpdated")
                .withArgs(account2.address);
        });

        it("should disallow a non-owner from setting dextoro", async () => {
            await onlyGivenAddressCanInvoke(
                supplySchedule.setDexToro,
                [account2.address],
                accounts,
                owner.address
            );
    ...
    describe("mint & setters", async () => {
        ...
        it("Assert proper inflation streams", async function () {
            ...
            expect(await dextoro.balanceOf(TREASURY_DAO_ADDRESS)).to.equal(
                expectedTreasurySupplyWithDivertedRewards.toBN()
            );

            expect(await dextoro.balanceOf(tradingRewards.address)).to.equal(
                expectedTradingRewardsStream.toBN()
            );

            expect(await dextoro.balanceOf(stakingRewards.address)).to.equal(
    ...
    describe("functions and modifiers", async () => {
        ...
        describe("mintable supply", async () => {
            ...
            async function checkMintedValues(
                ...
                // call updateMintValues to mimic dextoro issuing tokens
                await supplySchedule.setDexToro(owner.address);
