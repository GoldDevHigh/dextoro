...
describe('Mint', () => {
	...
	it('No inflationary supply to mint', async () => {
		...
		expect(await dextoro.balanceOf(stakingRewards.address)).to.equal(0);
	});
	...
	it('Mint inflationary supply 1 week later', async () => {
		...
		expect(await dextoro.balanceOf(owner.address)).to.equal(0);
	...
	describe('Verify future supply', () => {
		...
		it('Mint rewards 1 week later', async () => {
			...
			await supplySchedule.mint();
			expect(await dextoro.totalSupply()).to.equal(expected.toBN());
		});
