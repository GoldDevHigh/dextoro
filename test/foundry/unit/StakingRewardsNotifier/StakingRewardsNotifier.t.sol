...
contract StakingRewardsNotifierTest is DefaultStakingV2Setup {
    ...
    function testCannotDeployWithZeroOwnerAddress() public {
        ...
        new StakingRewardsNotifier(address(0), address(dextoro), address(supplySchedule));
    }

    function testCannotDeployWithZeroDexToroAddress() public {
        vm.expectRevert(IStakingRewardsNotifier.ZeroAddress.selector);
    ...
    function testCannotDeployWithZeroSupplyScheduleAddress() public {
        vm.expectRevert(IStakingRewardsNotifier.ZeroAddress.selector);
        new StakingRewardsNotifier(address(this), address(dextoro), address(0));
    }
    ...
    function testNotifiableRewardAccumulatorMintSuccess() public {
        ...
        uint256 balanceBefore = dextoro.balanceOf(address(stakingRewardsV2));
        vm.warp(block.timestamp + 2 weeks);
        vm.expectEmit(true, true, true, true);
        emit RewardAdded(mintAmount);
        supplySchedule.mint();
        uint256 balanceAfter = dextoro.balanceOf(address(stakingRewardsV2));
    ...
    function testNotifiableRewardAccumulatorRetroactiveFundsSuccess() public {
        ...
        dextoro.transfer(address(rewardsNotifier), 1000 ether);
        vm.warp(block.timestamp + 1 weeks);
        uint256 balanceBefore = dextoro.balanceOf(address(stakingRewardsV2));
    ...
    function testFuzzNotifiableRewardAccumulatorAddFunds(
        ...
        uint256 mintAmount = 17_177_543_635_384_615_384_614;
        dextoro.transfer(address(rewardsNotifier), retroactive1);
        vm.warp(block.timestamp + 1 weeks);
        dextoro.transfer(address(rewardsNotifier), retroactive2);
        vm.warp(block.timestamp + 1 weeks);
        dextoro.transfer(address(rewardsNotifier), retroactive3);
        uint256 balanceBefore = dextoro.balanceOf(address(stakingRewardsV2));
        vm.expectEmit(true, true, true, true);
    ...
    function testNotifiableRewardAccumulatorEarlyVest() public {
        ...
        dextoro.transfer(address(0x1), 100_000 ether);
