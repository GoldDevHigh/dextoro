...
contract StakingRewardsV2Test is DefaultStakingV2Setup {
    ...
    function setUp() public override {
        ...
        vm.prank(treasury);
        dextoro.transfer(address(stakingRewardsV2), INITIAL_SUPPLY / 4);
    }
    ...
    function test_Token_Set() public {
        address token = address(stakingRewardsV2.dextoro());
        assertEq(token, address(dextoro));
    }
    ...
    function test_Only_Owner_Can_Call_recoverERC20() public {
        vm.prank(user1);
        vm.expectRevert("Ownable: caller is not the owner");
        stakingRewardsV2.recoverERC20(address(dextoro), 0);
    }
    ...
    function test_Cannot_Recover_Staking_Token() public {
        ...
        stakingRewardsV2.recoverERC20(address(dextoro), TEST_VALUE);
    }

    function test_Can_Recover_Non_Staking_Token() public {
        // create mockToken
        IERC20 mockToken = new DexToro(
            "Mock",
            "MOCK",
            INITIAL_SUPPLY,
            address(this),
    ...
    function test_stake_Increases_Token_Balance() public {
        uint256 initialBalance = dextoro.balanceOf(address(stakingRewardsV2));
        ...
        assertEq(dextoro.balanceOf(address(stakingRewardsV2)), initialBalance + TEST_VALUE);
    }
    ...
    function test_Escrow_Staking_Does_Not_Increase_Token_Balance() public {
        ...
        // check balance increased
        assertEq(dextoro.balanceOf(address(stakingRewardsV2)), initialBalance);
    }
    ...
    function test_exit() public {
        ...
        // transfer dextoro to this address
        vm.prank(treasury);
        dextoro.transfer(address(this), nonEscrowStakedBalance);

        // stake non-escrowed dextoro
        dextoro.approve(address(stakingRewardsV2), nonEscrowStakedBalance);
        stakingRewardsV2.stake(nonEscrowStakedBalance);

        // stake escrowed dextoro
        ...
        assertEq(dextoro.balanceOf(address(this)), nonEscrowStakedBalance);
    }
    ...
    function test_rewardRate_Should_Increase_If_New_Rewards_Come_Before_Duration_Ends() public {
        ...
        // send dextoro to stakingRewardsV2 contract
        vm.prank(treasury);
        dextoro.transfer(address(stakingRewardsV2), totalToDistribute);
    ...
    function test_getRewardForDuration() public {
        vm.prank(treasury);
        dextoro.transfer(address(stakingRewardsV2), TEST_VALUE);
    ...
    function test_Unstaking_Token_And_Staking_Balance_Updates() public {
        ...
        uint256 initialTokenBalance = dextoro.balanceOf(address(this));
        uint256 initialStakingBalance = stakingRewardsV2.balanceOf(address(this));

        vm.warp(block.timestamp + 2 weeks);
        stakingRewardsV2.unstake(TEST_VALUE);

        uint256 finalTokenBalance = dextoro.balanceOf(address(this));
    ...
    function test_unstakeEscrow_Does_Not_Change_Token_Balances() public {
        ...
        uint256 initialTokenBalance = dextoro.balanceOf(address(this));
        uint256 initialEscrowTokenBalance = dextoro.balanceOf(address(rewardEscrowV2));
        ...
        uint256 finalTokenBalance = dextoro.balanceOf(address(this));
        uint256 finalEscrowTokenBalance = dextoro.balanceOf(address(rewardEscrowV2));
    ...
    function test_exit_Should_Retrieve_All_earned() public {
        ...
        // get initial values
        uint256 initialRewardBalance = dextoro.balanceOf(address(this));
        ...
        // get final values
        uint256 finalRewardBalance = dextoro.balanceOf(address(this));
