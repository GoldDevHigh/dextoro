...
// StakingRewards: fund with DTORO and set the rewards
const fundAndSetStakingRewards = async (value: number) => {
    // send/fund StakingRewards
    dextoro.connect(TREASURY_DAO).transfer(stakingRewards.address, value);

    // set duration/epoch/period
    await stakingRewards.setRewardsDuration(value);

    // set rewards per duration/epoch/period
    // (i.e. 1 DTORO per second since value is used for both duration and reward amount)
...
const loadSetup = () => {
    before("Deploy contracts", async () => {
        ...
        rewardEscrow = await RewardEscrow.deploy(owner.address, dextoro.address);
...
describe("Stake (fork)", () => {
    describe("Staking", async () => {
        ...
        it("Stake and claim rewards", async () => {
            ...
            expect(await dextoro.balanceOf(addr1.address)).to.equal(0);

            // transfer DTORO to addr1
            await expect(() =>
                dextoro.connect(TREASURY_DAO).transfer(addr1.address, TEST_VALUE)
            ).to.changeTokenBalance(dextoro, addr1, TEST_VALUE);

            // increase DTORO allowance for stakingRewards
            await dextoro
                .connect(addr1)
        ...
        it("Stake, Wait, and then Exit", async () => {
            // initial balance should be 0
            expect(await dextoro.balanceOf(addr2.address)).to.equal(0);

            // transfer DTORO to addr2
            await expect(() =>
                dextoro.connect(TREASURY_DAO).transfer(addr2.address, TEST_VALUE)
            ).to.changeTokenBalance(dextoro, addr2, TEST_VALUE);

            // set the rewards for the next duration/epoch/period
            await fundAndSetStakingRewards(SECONDS_IN_WEEK);

            // increase DTORO allowance for stakingRewards and stake
            await dextoro
                .connect(addr2)
                .approve(stakingRewards.address, TEST_VALUE);
            ...
            expect(await dextoro.balanceOf(addr2.address)).to.equal(TEST_VALUE);
        });
    });
    ...
    describe("Escrow staking", async () => {
        ...
        before("Create new escrow entry", async () => {
            ...
            // increase DTORO allowance for rewardEscrow and stake
            await dextoro
                .connect(addr1)
        ...
        it("Stake escrowed dextoro", async () => {
        ...
        it("Unstake escrowed dextoro", async () => {
            // initial balance(s) should be 0
            expect(await dextoro.balanceOf(addr1.address)).to.equal(0);
            expect(await dextoro.balanceOf(addr2.address)).to.equal(0);

            // establish amount of staked DTORO
