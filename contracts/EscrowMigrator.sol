    ...
    UUPSUpgradeable
{
    ...
    /// @notice Mapping of $DTORO paid so far for the migration for each account
    ...
    constructor(
        ...
        if (_dextoro == address(0)) revert ZeroAddress();
    ...
    /// @notice The user must vest any registered entries and approve this contract to spend `toPay` amount of liquid $DTORO
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < _entryIDs.length; i++) {
            ...
            dextoro.transfer(address(rewardEscrowV2), originalEscrowAmount);
    ...
    function _payForMigration(address _account) internal {
        ...
        if (toPayNow > 0) {
            dextoro.transferFrom(msg.sender, address(this), toPayNow);
    ...
    function recoverExcessFunds() external onlyOwner {
        ...
        uint256 balance = dextoro.balanceOf(address(this));
        if (balance > leaveInContract) {
            dextoro.transfer(treasuryDAO, balance - leaveInContract);
        }
    }
